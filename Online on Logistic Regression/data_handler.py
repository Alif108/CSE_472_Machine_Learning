...
# csv_file_path = "C:/Users/Asus/OneDrive/Desktop/1705108/Raisin_Dataset/Raisin_Dataset.xlsx"
csv_file_path = "Occupancy_Dataset/datatraining.txt"
...
def load_dataset():
    ...
    :return:
    """
    # dataset = pd.read_excel(csv_file_path)

    dataset = pd.read_csv(csv_file_path)
    dataset = dataset.loc[:, dataset.columns != 'date']     # remove date column
    
    dataset = (dataset-dataset.min())/(dataset.max()-dataset.min())     # normalize data
...
def load_test_data(filepath):
    ...
    test_data = test_data.loc[:, test_data.columns != 'date']       # remove date column

    test_data = (test_data-test_data.min())/(test_data.max()-test_data.min())   # normalize data

    X_test = test_data.iloc[:, :-1].values     # all rows, all columns except last one (exclude class column)
    y_test = test_data.iloc[:, -1].values      # all rows, last column (class column)
